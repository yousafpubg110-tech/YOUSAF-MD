...
import { handleAwayMessage } from '../plugins/away.js';
...
async function handleMessages(sock, messageUpdate) {
    try {
        ...
        if (!message.key.fromMe) {
            ...
            if (replied)
                return;
            // Away mode: sends the owner's custom "I'm offline" message to a
            // private chat at most once per person per cooldown window.
            // Never blocks command processing — it just also notifies them.
            await handleAwayMessage(sock, chatId, message, senderId);
        }
        const senderIsOwnerOrSudo = await isOwnerOrSudo(senderId, sock, chatId);
        const isOwnerOrSudoCheck = message.key.fromMe || senderIsOwnerOrSudo;
        if (message.message?.buttonsResponseMessage) {
            ...
            if (buttonId === 'channel') {
                await sock.sendMessage(chatId, {
                    text: '*Join our Channel:*\n[https://whatsapp.com/channel/0029Vb3Uzps6buMH2RvGef0j](https://whatsapp.com/channel/0029Vb3Uzps6buMH2RvGef0j)'
                }, { quoted: message });
                return;
            }
