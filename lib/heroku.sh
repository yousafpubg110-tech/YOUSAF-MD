#!/bin/bash
# YOUSAF-MD Heroku One-Click Deployer
# Usage: bash <(curl -s https://raw.githubusercontent.com/yousafpubg110-tech/YOUSAF-MD/main/lib/heroku.sh)
...
echo -e "${CYAN}"
echo "╔═══════════════════════════════════════╗"
echo "║       YOUSAF-MD Heroku Deployer         ║"
echo "║         by Muhammad Yousaf Baloch             ║"
echo "╚═══════════════════════════════════════╝"
echo -e "${NC}"
...
# App name
read -p "$(echo -e ${CYAN}App name (e.g. my-yousaf-md): ${NC})" APP_NAME
APP_NAME=${APP_NAME:-yousaf-md-bot}

# Session ID
read -p "$(echo -e ${CYAN}Session ID (yousafpubg110-tech/YOUSAF-MD_xxxxx): ${NC})" SESSION_ID
...
# Bot name
read -p "$(echo -e ${CYAN}Bot name (default: YOUSAF-MD): ${NC})" BOT_NAME
BOT_NAME=${BOT_NAME:-YOUSAF-MD}
...
if [ ! -f "heroku.yml" ]; then
    echo -e "${YELLOW}📦 Cloning YOUSAF-MD repo...${NC}"
    git clone https://github.com/yousafpubg110-tech/YOUSAF-MD yousaf-md-deploy
    cd yousaf-md-deploy
fi
