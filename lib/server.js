...
app.get('/', (req, res) => {
    ...
    // Reflect the REAL WhatsApp connection state, not just "the web
    // server process is running". socketRegistry.sock.user only exists
    // once Baileys has actually finished pairing/logging in.
    const isConnected = !!(socketRegistry.sock && socketRegistry.sock.user);
    const statusLabel = isConnected ? 'WHATSAPP CONNECTED' : 'WAITING FOR PAIRING';
    const statusColor = isConnected ? '#25d366' : '#f59e0b';
    const actionHtml = isConnected
        ? ''
        : `<a href="/pair" style="display:block;margin-top:20px;background:#25d366;color:#0f172a;font-weight:800;padding:14px;border-radius:12px;text-decoration:none;">🔗 Click here to pair your WhatsApp number</a>`;
    ...
    <head>
        ...
        <style>
            :root { --primary: ${statusColor}; --bg: #0f172a; --card-bg: rgba(30, 41, 59, 0.7); }
            body { 
    ...
    <body>
        <div class="container">
            <div class="status-badge"><span class="dot"></span> ${statusLabel}</div>
