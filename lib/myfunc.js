...
export const generateProfilePicture = async (buffer) => {
    // Lazy-loaded: sharp is statically imported by index.js via this file's
    // smsg export, so an eager top-level `import sharp` here crashes the
    // ENTIRE bot on startup on platforms where sharp's native binary isn't
    // available (e.g. Termux/Android ARM64 without libvips). Only load it
    // when this specific function is actually called.
...
export const reSize = async (buffer, ukur1, ukur2) => {
    const { default: sharp } = await import('sharp');
    return sharp(buffer).resize(ukur1, ukur2).jpeg().toBuffer();
};
// ─── Size ─────────────────────────────────────────────────────────────────────
...
export const smsg = (YousafBot, m, store) => {
    if (!m)
        return m;
    ...
    if (m.key) {
        ...
        m.sender = YousafBot.decodeJid(m.fromMe && YousafBot.user.id || m.participant || m.key.participant || m.chat || '');
        if (m.isGroup)
            m.participant = YousafBot.decodeJid(m.key.participant) || '';
    }
    if (m.message) {
        ...
        if (m.quoted) {
            ...
            m.quoted.sender = YousafBot.decodeJid(m.msg.contextInfo.participant);
            m.quoted.fromMe = m.quoted.sender === (YousafBot.user && YousafBot.user.id);
            m.quoted.text = m.quoted.text || m.quoted.caption || m.quoted.conversation || m.quoted.contentText || m.quoted.selectedDisplayText || m.quoted.title || '';
            m.quoted.mentionedJid = m.msg.contextInfo ? m.msg.contextInfo.mentionedJid : [];
            m.getQuotedObj = m.getQuotedMessage = async () => {
                if (!m.quoted.id)
                    return false;
                const q = await store.loadMessage(m.chat, m.quoted.id, YousafBot);
                return smsg(YousafBot, q, store);
            };
            ...
            m.quoted.delete = () => YousafBot.sendMessage(m.quoted.chat, { delete: vM.key });
            m.quoted.copyNForward = (jid, forceForward = false, options = {}) => YousafBot.copyNForward(jid, vM, forceForward, options);
            m.quoted.download = () => YousafBot.downloadMediaMessage(m.quoted);
        }
    }
    if (m.msg?.url)
        m.download = () => YousafBot.downloadMediaMessage(m.msg);
    m.text = m.msg?.text || m.msg?.caption || m.message?.conversation || m.msg?.contentText || m.msg?.selectedDisplayText || m.msg?.title || '';
    m.reply = (text, chatId = m.chat, options = {}) => Buffer.isBuffer(text)
        ? YousafBot.sendMedia(chatId, text, 'file', '', m, { ...options })
        : YousafBot.sendText(chatId, text, m, { ...options });
    m.copy = () => smsg(YousafBot, M.fromObject(M.toObject(m)), store);
    m.copyNForward = (jid = m.chat, forceForward = false, options = {}) => YousafBot.copyNForward(jid, m, forceForward, options);
    return m;
};
