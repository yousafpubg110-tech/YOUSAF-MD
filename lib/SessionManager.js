...
// FIXED: single-boot guarantee, phone validation, clean socket teardown
async function generatePairingCode(phoneNumber) {
  ...
  // Wait for WebSocket handshake before requesting code
  ...
  // FIXED: boot full instance ONCE on pairing success, then discard temp socket
...
async function _sendWelcome(sock, jid, instanceId) {
  try {
    await new Promise(r => setTimeout(r, 3000)); // let connection settle
