...
//   Personal Instance — Single user, no central DB
...
async function generatePairingCode(phone) {
  ...
  // Wait for 'connecting' state then request pairing code after
  // 1500ms delay — this gives WhatsApp handshake time to settle
  ...
  // ── Handle connection events after code is shown ────────
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'open') {
      ...
      // Send welcome message — NO session ID
      try {
...
// ── RESTORE SESSION (after disconnect or restart) ─────────
...
// Restores any existing session when Heroku dyno restarts
