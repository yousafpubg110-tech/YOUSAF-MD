...
async function generatePairingCode(phone) {
  ...
  // Request pairing code at the correct moment:
  // Baileys fires connection.update with connection='connecting' TWICE —
  // first time is too early. We must wait until sock.authState.creds.me
  // is NOT set (fresh login) AND connection is 'connecting' for the
  // second time, OR simply use a small delay after socket creation.
  // The most reliable method proven to work: request ONCE after socket
  // initialises but BEFORE it reaches 'open' — using a 1500ms delay
  // after the FIRST 'connecting' event (giving handshake time to settle).
  const code = await new Promise((resolve, reject) => {
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'connecting' && !connectSeen && !codeSent) {
        ...
        // Wait 1500ms after first 'connecting' — handshake settles in this window
        setTimeout(async () => {
          if (codeSent) return;
          codeSent = true;
